"""
AWS ListenBrainz Ingestion Pipeline
- Reads tar.zst dumps from S3
- Processes into daily aggregates
- Writes to RDS PostgreSQL
- Computes stats using PySpark

Run on EC2 instance with IAM role granting S3 + RDS access
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/listenbrainz_ingest.log'),
...
# ============================================================================
# CONFIG - EDIT THESE FOR YOUR AWS ENVIRONMENT
# ============================================================================

# S3 Configuration
S3_BUCKET = "your-bucket-name"  # Your S3 bucket with dumps
S3_DUMP_KEY = "musicbrainz/listenbrainz-listens-dump-2428-20260213-000003-incremental.tar.zst"  # Path in S3
TEMP_DIR = "/tmp/listenbrainz"  # Where to extract dumps on EC2
...
RDS_USER = "postgres"
RDS_PASSWORD = os.getenv("RDS_PASSWORD")  # Set via environment variable or Secrets Manager

# AWS Region
AWS_REGION = "us-east-1"  # Change if your RDS is in different region
...
def download_s3_dump(bucket, key, local_path):
    """Download tar.zst dump from S3 to local EC2 storage."""
...
def parse_dump(dump_path):
    ...
    logger.info(f"Parsing {dump_path}")
    ...
    logger.info(f"Parse summary: {summary}")
...
def upsert_data(conn, track_daily, artist_daily, track_info, artist_info, dump_key):
    ...
    with conn, conn.cursor() as cur:
        ...
        logger.info(f"Upserted {len(artist_rows)} artist records")
        ...
        logger.info(f"Upserted {len(track_rows)} track records")
        ...
        logger.info(f"Upserted {len(track_daily_rows)} track_daily records")
        ...
        logger.info(f"Upserted {len(artist_daily_rows)} artist_daily records")
...
def compute_stats_with_spark(rds_host, rds_port, rds_db, rds_user, rds_password):
    ...
    try:
        ...
        artist_df = artist_df.withColumn("cumulative_listen_count", F.sum("listen_count").over(artist_window_unbounded))
        artist_df = artist_df.withColumn("listen_count_past_7_days", F.sum("listen_count").over(artist_window_7d))
        artist_df = artist_df.withColumn("listen_count_past_30_days", F.sum("listen_count").over(artist_window_30d))
        
        artist_window_lag = Window.partitionBy("artist_mbid").orderBy("day")
        artist_df = artist_df.withColumn("cumulative_yesterday", F.lag("cumulative_listen_count", 1).over(artist_window_lag))
        artist_df = artist_df.withColumn(
            "growth_rate",
            F.when(F.col("cumulative_yesterday").isNotNull() & (F.col("cumulative_yesterday") > 0),
                   F.col("listen_count") / F.col("cumulative_yesterday")).otherwise(None)
        )
        
        artist_df = artist_df.withColumn("growth_percentile", F.percent_rank().over(Window.partitionBy("day").orderBy(F.col("growth_rate").asc_nulls_first())))
        artist_df = artist_df.withColumn("listen_pctl_past_7_days", F.percent_rank().over(Window.partitionBy("day").orderBy(F.col("listen_count_past_7_days"))))
        artist_df = artist_df.withColumn("listen_pctl_past_30_days", F.percent_rank().over(Window.partitionBy("day").orderBy(F.col("listen_count_past_30_days"))))
        
        artist_stats = artist_df.select("day", "artist_mbid", "growth_percentile", "cumulative_listen_count", 
        ...
        logger.info(f"Wrote {artist_stats.count():,} artist stats")
        ...
        track_df = track_df.withColumn("cumulative_listen_count", F.sum("listen_count").over(track_window_unbounded))
        track_df = track_df.withColumn("listen_count_past_7_days", F.sum("listen_count").over(track_window_7d))
        track_df = track_df.withColumn("listen_count_past_30_days", F.sum("listen_count").over(track_window_30d))
        
        track_window_lag = Window.partitionBy("recording_id").orderBy("day")
        track_df = track_df.withColumn("cumulative_yesterday", F.lag("cumulative_listen_count", 1).over(track_window_lag))
        ...
        track_df = track_df.withColumn("growth_percentile", F.percent_rank().over(Window.partitionBy("day").orderBy(F.col("growth_rate").asc_nulls_first())))
        track_df = track_df.withColumn("listen_pctl_past_7_days", F.percent_rank().over(Window.partitionBy("day").orderBy(F.col("listen_count_past_7_days"))))
        track_df = track_df.withColumn("listen_pctl_past_30_days", F.percent_rank().over(Window.partitionBy("day").orderBy(F.col("listen_count_past_30_days"))))
        
        track_stats = track_df.select("day", "recording_id", "growth_percentile", "cumulative_listen_count",
                                     "listen_count_past_7_days", "listen_pctl_past_7_days",
                                     "listen_count_past_30_days", "listen_pctl_past_30_days")
        
        track_stats.write.jdbc(url=jdbc_url, table="track_daily_stats", mode="overwrite", properties=jdbc_props)
        logger.info(f"Wrote {track_stats.count():,} track stats")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Starting ListenBrainz AWS Ingestion Pipeline")
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info(f"Connected to RDS: {RDS_USER}@{RDS_HOST}:{RDS_PORT}/{RDS_DATABASE}")
    except Exception as e:
